...
class visimisiController extends Controller
{
    ...
    public function getDataJson()
    {
        ...
        $data = VisiMisi::select("id","visi","misi")->orderby('created_at', 'desc'); // diurutkan berdasarkan created_at dengan urutan terbaru ke terlama
        return DataTables::eloquent($data)
            ...
            ->addColumn('deskripsi', function($data) {
                //str limit untuk menbatasi string yang terlalu panjang
                return strlen($data->deskripsi) > 20 ? substr($data->deskripsi, 0, 50) . '...' : $data->deskripsi;
            })
            ->rawColumns(['opsi', 'deskripsi']) // render html di dalam datatable
            ->make();
    }
