            ...
            <div class="row mb-5 justify-content-lg-end justify-content-center p-2">
                ...
                <div class="col-lg-3 col-md-6 col-sm-12">
                    <div class="input-group">
                        ...
                        <input type="text" id="search" class="form-control" onkeyup="filter()" onkeydown="filter()" placeholder="Search..." autocomplete="off">
                    </div>
                </div>
            </div>
            <div class="row mx-5 mb-2 align-items-scretch" id="podcast-container">
            </div>
...
@section('scripts')
    <script>
        ...
        function podcast(search, page) {
            var beritaContainer = $('#podcast-container');
            var noDataSection = $('#no-data-section');
            $.ajax({
                url: "{{ route('video.json.video') }}?search=" + search + "&page=" + page,
                type: 'GET',
                dataType: 'json',
        ...
        function updatePagination(response) {
            ...
            if (lastPage > 1) {
                ...
                // Previous page link
                if (currentPage > 1) {
                    paginationHtml += '<li class="page-item"><a class="page-link" href="#" onclick="podcast(\'' + response.text + '\', ' + (currentPage - 1) + ')">Previous</a></li>';
                }

                // Page links
                for (var i = 1; i <= lastPage; i++) {
                    paginationHtml += '<li class="page-item ' + (i === currentPage ? 'active' : '') + '"><a class="page-link" href="#" onclick="podcast(\'' + response.text + '\', ' + i + ')">' + i + '</a></li>';
                }

                // Next page link
                if (currentPage < lastPage) {
                    paginationHtml += '<li class="page-item"><a class="page-link" href="#" onclick="podcast(\'' + response.text + '\', ' + (currentPage + 1) + ')">Next</a></li>';
                }
        ...
        function displayDataInBlade(data) {
            ...
            $.ajax({
                ...
                url: "{{ route('video.blade') }}",
                data: { data: dataJson }, // Kirim data ke Blade
                success: function(response) {
                    $('#podcast-container').html(response); // Tampilkan hasil dari Blade
                },
                error: function(xhr, status, error) {
