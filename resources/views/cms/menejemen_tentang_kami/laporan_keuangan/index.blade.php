...
@section('judul', 'Laporan Keuangan Bulanan')
					...
					<div class="card card-flush h-xl-100">
						<div class="card-header pt-5">
							<h3 class="card-title align-items-start flex-column">
								<span class="card-label fw-bolder text-dark">Data Laporan Keuangan Bulanan</span>
							</h3>
                            <div class="card-toolbar">
                            <select class="form-select mb-2 w-100 mw-250px" data-control="select2" data-hide-search="true" name="tahun" id="tahun" onchange="filter_tanggal(); tableDraw();">
				...
				<div class="col-xl-12 mb-xl-10">
					<div class="card card-flush">
						<div class="card-header mt-6">
							<div class="card-title">
								<div class="d-flex align-items-center position-relative my-1 me-5">
									<p class="card-label fw-bolder text-dark">Data Laporan Keuangan Bulanan</p>
								</div>
							</div>
                            <div class="card-toolbar">
						...
						<div class="card-body pt-0">
							<table class="table align-middle table-row-dashed fs-6 gy-5 mb-0" id="tbl-data-laporan">
								<thead>
									<tr class="text-start text-black-400 fw-bolder fs-7 text-uppercase gs-0">
										<th class="min-w-50px">No</th>
										<th class="min-w-150px">Bulan</th>
...
<script>
	$(function() {
		...
		$("#tbl-data-laporan").DataTable({
            ...
            ajax: {
                ...
                url: "{{ route('laporan-keuangan-bulanan.getDataJson') }}",
                type: "POST",
                data: function(data) {
                    data.date = $("#tahun").val();
                }
	...
	function chartTotalTahunan(date) {
        $.ajax({
            url: "{{ route('laporan-keuangan-bulanan.chartLaporan') }}?date=" + date,
            type: 'GET',
            dataType: 'json',
    ...
    function tableDraw() {
        $("#tbl-data-laporan").DataTable().draw(); // Memuat ulang data dari server dan menggambar ulang tabel
    };
    ...
    $('.btn-tambah-laporan').on('click', function() {
        $.ajax({
            type: "GET",
            url: "{{ route('laporan-keuangan-bulanan.create', '') }}" + '/',
            success: function(response) {
    ...
    function btn_edit_laporan(id) {
        $.ajax({
            type: "GET",
            url: "{{ route('laporan-keuangan-bulanan.edit', '') }}" + '/' + id,
            success: function(response) {
    ...
    function btn_hapus_laporan(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location = "{{ route('laporan-keuangan-bulanan.destroy', '') }}" + '/' + id;
            }
        });
    }
